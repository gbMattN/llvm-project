...
__tysan_instrument_mem_inst(char *dest, char *src, uint64_t size,
                            bool needsMemMove) {
  uint64_t *shadowData = (uint64_t *)*shadow_for(dest);

  if (src == nullptr) {
    memset((char *)shadowData, 0, size << PtrShift());
    return;
  }
  ...
  if (needsMemMove) {
    memmove(shadowData, srcShadow, size << PtrShift());
  } else {
    memcpy(shadowData, srcShadow, size << PtrShift());
  }
}
